if not LoaderTCRandChar then
    require'LoaderTCRandChar'
end
...
this.main = function (opt)
    ...
    if dataMask.tr == true then
        ...
        tr = LoaderTCRandChar(fntr, opt.batSize, opt.seqLength, v_char)
    end

    if dataMask.val == true then
        local fnval = path.join(dataPath, 'te.t7')
        print('val data loader')
        val = LoaderTCRandChar(fnval, opt.batSize, opt.seqLength, v_char)
    end

    if dataMask.te == true then
        local fnte = path.join(dataPath, 'te.t7')
        print('test data loader')
        te = LoaderTCRandChar(fnte, opt.batSize, opt.seqLength, v_char)
    end
