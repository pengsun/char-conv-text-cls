...
local function get_rand_char_seq(xchar, seqlen, char_null)
	assert(char_null, "char_null not defined")
	local xx = torch.ByteTensor(seqlen):fill(char_null)

	-- the source range
	local srcBeg = math.random(xchar:numel()) - seqlen
	srcBeg = math.max(srcBeg, 1)
...
--- class def
local LoaderTCRandChar = torch.class('LoaderTCRandChar')

function LoaderTCRandChar:__init(ffnData, batSize, seqLength, vocab)
...
function LoaderTCRandChar:reset_batch_pointer()
	self.iBat = 0
	self:reset_order()
end

function LoaderTCRandChar:cuda()
	self.isCuda = true
end

function LoaderTCRandChar:cpu()
	self.isCuda = false
end

function LoaderTCRandChar:training()
	self.flagOrderRand = true
	self:reset_batch_pointer()
end

function LoaderTCRandChar:evaluate()
	self.flagOrderRand = false
	self:reset_batch_pointer()
end

function LoaderTCRandChar:next_batch()
	...
	for i = 1, self.batSize do
		...
		xx[i]:copy( get_rand_char_seq(self.x[ind], self.seqLength, self.char_null) )
		yy[i] = self.y[ind]
	end
...
function LoaderTCRandChar:num_batches()
	return self.numBat -- truncate!
end

--- helper methods
function LoaderTCRandChar:reset_order()
